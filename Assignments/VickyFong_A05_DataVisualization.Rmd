...
temp <- ntl %>%
  ...
  scale_x_discrete(limits=c('5','6','7','8','9','10','11'), labels=NULL) +
...
needles <- neon %>%
  ...
  select(collectDate,functionalGroup,dryMass,nlcdClass) %>%
  mutate(date = as.factor(collectDate))

needles %>%
  ggplot(aes(x=date, y=dryMass, fill=nlcdClass))+
  geom_boxplot() +
  labs(x='Date', y='Dry Mass (g)', fill='NLCD Class') +
...
needles %>%
  ...
  labs(x='Date', y='Dry Mass (g)', title='Dry Mass of Needles in 3 NLCD') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
...
> Answer: The facet wrap plots in question 7 is more effective as the x-axis spans a large time frame and thus it was very difficult to see the data with three NLCD classes. Since the collection dates in the x-axis is the same across NLCD classes, separating the data into three facet wrap graphs allows us to better visualize the temporal trends of needle dry mass in each type of NLCD. 