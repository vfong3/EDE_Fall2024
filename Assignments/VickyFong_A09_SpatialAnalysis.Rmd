...
> ANSWER: 
> The EPSG code is 4269. This is a geographic coordinate reference system. The datum is North America 1983.
...
gage <- read_csv(here('Data/Raw/NWIS_SiteInfo_NE_RAW.csv'))
...
> ANSWER:
> The "dec_long_va" column contains x coordinates and the "dec_lat_va" column contains y coordinates.
...
>ANSWER: 
> A new field called 'geometry' was added that classified each data point as a 'point' with the latitude and longitude combined, which deleted the individual 'dec_long_va' and 'dec_lat_va' columns.
...
ggplot() +
  geom_sf(data = counties,fill='pink') + 
  geom_sf(data = gage_sf, color='black') +
  labs(title = "NWIS Gage Locations in Nebraska", subtitle = "Vicky Fong")
...
flow <- read_csv(here('Data/Raw/NWIS_SiteFlowData_NE_RAW.csv'))
...
joined_sf <- left_join(gage_sf, flow, by = "site_no")

joined_sf <- joined_sf %>% na.omit()
...
counties_sf_utm <- st_transform(counties, epgs = 32614)
gage_sf_utm <- st_transform(gage_sf, epgs = 32614)
...
gage_intersect <- gage_sf_utm %>%
  st_filter(lancaster, .predicate = st_intersects)


#24 Spatially select gages within 15 k of the selected county
buffer <-st_buffer(lancaster,15000)

gage_15km <- gage_sf_utm %>%
  st_filter(buffer, .predicate = st_intersects)

#25 Plot
ggplot()+
  geom_sf(data = counties_sf_utm, aes(fill="A"),alpha = 0.5, show.legend = "polygon") +
  geom_sf(data = lancaster, aes(fill="B"), show.legend = "polygon") +
  geom_sf(data = gage_15km, aes(color="C"), size=0.5, show.legend = "point") +
  geom_sf(data = gage_intersect, aes(color="D"), size=0.5, show.legend = "point") +
  scale_fill_manual(values = c("A" = "lightyellow", "B" = "pink"), 
                    labels = c("Nebaska counties", "Lancaster county"), 
                    name = "Legend",
                    guide = guide_legend(override.aes = 
                                           list(linetype = "blank", shape = NA))) +
  scale_color_manual(values = c("C" = "blue", "D" = "red"),
                     labels = c("Within 15 km of Lancaster", "Inside Lancaster"),
                     name = "Gage Stations") +
  labs(title = "Gage Stations near Lancaster county, Nebraska")
```
